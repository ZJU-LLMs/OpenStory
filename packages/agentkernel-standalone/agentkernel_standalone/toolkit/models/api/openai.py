...
class OpenAIProvider(ChatModelProvider, EmbeddingModelProvider):
    ...
    def parse_response(self, response: str) -> Optional[str]:
        ...
        try:
            ...
            # Extract and log usage statistics if available
            usage = res.get("usage", {})
            if usage:
                prompt_tokens = usage.get("prompt_tokens", 0)
                completion_tokens = usage.get("completion_tokens", 0)
                total_tokens = usage.get("total_tokens", 0)
                logger.info(f"【LLM API Metric】Model: {self.model} | Prompt Tokens: {prompt_tokens} | Completion Tokens: {completion_tokens} | Total Tokens: {total_tokens}")
