...
class EnvironmentComponent(ABC):
    ...
    async def save_to_db(self) -> None:
        """
        Save the registered plugin state when the plugin supports persistence.
        """
        if self._plugin:
            try:
                ...
                logger.info(
                    "Plugin '%s' on environment component '%s' does not implement 'save_to_db'. Skipping.",
                    self._plugin.__class__.__name__,
                    self.COMPONENT_NAME,
                )
            except Exception as e:
                logger.error(
                    "Error saving state for plugin '%s' on environment component '%s': %s",
                    self._plugin.__class__.__name__,
                    self.COMPONENT_NAME,
                    e,
                    exc_info=True,
    ...
    async def load_from_db(self) -> None:
        """
        Load the registered plugin state when the plugin supports persistence.
        """
        if self._plugin:
            try:
                ...
                logger.info(
                    "Plugin '%s' on environment component '%s' does not implement 'load_from_db'. Skipping.",
                    self._plugin.__class__.__name__,
                    self.COMPONENT_NAME,
                )
            except Exception as e:
                logger.error(
                    "Error loading state for plugin '%s' on environment component '%s': %s",
                    self._plugin.__class__.__name__,
                    self.COMPONENT_NAME,
                    e,
                    exc_info=True,
