...
async function enterStoryModeSetup() {
  if (storyLaunchInFlight) return;
  storyLaunchInFlight = true;
  refreshStoryServiceStatus('starting', '正在请求启动剧情模式服务…');

  const btn = document.querySelector('.mode-buttons-group .mode-btn');
  const originalText = btn ? btn.innerHTML : '';
  if (btn) {
    btn.innerHTML = '<span>剧情模式启动中…</span>';
    btn.disabled = true;
  }
  ...
  try {
    ...
    if (data.status === 'port_busy') {
      refreshStoryServiceStatus('port_busy', data.message || '8001 端口被其他进程占用');
      alert(data.message || '剧情模式端口 8001 已被占用，请先关闭旧的 story 服务。');
      if (btn) btn.innerHTML = originalText;
      if (btn) btn.disabled = false;
      storyLaunchInFlight = false;
      return;
    }
    if (data.status === 'timeout') {
      await refreshStoryServiceStatus();
    ...
    if (data.status === 'failed') {
      await refreshStoryServiceStatus();
      alert(data.message || '剧情模式服务启动失败，请检查后端日志。');
      if (btn) btn.innerHTML = originalText;
      if (btn) btn.disabled = false;
      storyLaunchInFlight = false;
      return;
...
function applyStoryServiceStatus(status, message) {
  const el = document.getElementById('storyServiceStatus');
  if (!el) return;
  el.className = `story-service-status story-service-status-${status || 'pending'}`;
  el.textContent = message || '剧情模式服务状态未知';
}

async function refreshStoryServiceStatus(overrideStatus = null, overrideMessage = null) {
  if (overrideStatus || overrideMessage) {
    applyStoryServiceStatus(overrideStatus || 'pending', overrideMessage || '剧情模式服务状态检查中…');
    return;
  }
  try {
    const resp = await fetch('/api/story-status', { cache: 'no-store' });
    const data = await resp.json();
    applyStoryServiceStatus(data.status || 'pending', data.message || '剧情模式服务状态未知');
  } catch (e) {
    applyStoryServiceStatus('pending', '无法获取剧情模式服务状态');
  }
}

function startStoryServiceStatusPolling() {
  if (storyStatusPollTimer) return;
  storyStatusPollTimer = setInterval(() => {
    const modeScreen = document.getElementById('modeSelectionScreen');
    if (modeScreen && !modeScreen.classList.contains('hidden')) {
      refreshStoryServiceStatus();
    }
  }, 5000);
}
...
document.addEventListener('DOMContentLoaded', () => {
  refreshStoryServiceStatus();
  startStoryServiceStatusPolling();
...
function updateHistoryModeBanner() {
  ...
  if (isViewingHistory && viewingTick !== -1) {
    ...
    const msg = isAtCurrentFrontier
      ? `⏩ 正在查看 Tick ${viewingTick} · 点击"开始模拟"将在此时间线继续`
      : isAtOtherFrontier
        ? `↩ 正在查看 Tick ${viewingTick} · 点击"开始模拟"将切换至此时间线并继续`
