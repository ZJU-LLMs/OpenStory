...
FIXED_PIXEL_ART_STYLE = (
    ...
    "使用有限且协调的调色板、统一清晰的深色像素轮廓、平整连续的填充色和硬边块状阴影。"
    "建筑、设施、树木和景物采用简化、圆润、容易辨认的卡通造型，每个物体内部只使用二到四档明确色阶。"
    "屋顶、墙面、地表、水面和树冠保持大片干净色面，边缘呈清晰有节奏的像素阶梯。"
...
def compose_background_prompt(
    ...
    prompt_lines = [
        "根据输入的布局控制底板和透明保护遮罩，一次性生成连续、完整的世界地图背景。地点矩形区域和道路网格已经从可编辑区域中精确扣除。",
        (
            "布局底板中每个完整的矩形建筑体块都表示一处已经被未来建筑、院落或场所占用的位置，狭长连通走廊表示"
        ...
        (
            "蒙版已经由接口和后端精确处理，不得重画、复制、移动、扩展或近似复现其形状。必须用连续环境填充可编辑区域，不要求填满。"
    ...
    prompt_lines.extend(
        [
            "构图要求：",
            "1. 在可编辑区域中连续规划地表、水体、低矮植被、通用建筑、公共设施、交通元素、生活器具和景观装饰，使环境关系自然连续，不要拆成互不关联的独立装饰块。不要求填满可编辑区域",
            "2. 文明世界必须以符合时代的建筑、设施和生活风物作为主要视觉信息，自然景观只作为辅助。所有元素必须符合当前世界的时代与文化，不要混用其他时代。",
            ...
            "5. 地点矩形边界外侧紧邻区域只绘制能够自然延续到未来地点素材边缘的地表、水岸、草地、低矮花丛或小型地面装饰。不要沿矩形边缘排列半截屋顶、断墙、门廊、桥身、大型树冠或大型设施；这只是构图限制，不得人为扩大或改变保护区域。",
            "6. 视觉丰富度来自完整风物、明确轮廓和有限色阶，而不是写实材质或细碎纹理。屋瓦只画概括性分区，墙面只保留少量结构线，草地使用连续纯净色块，树冠使用少量成组像素簇。",
